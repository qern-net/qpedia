...
| # | Item | Repo | Status |
|---|---|---|---|
| 0.1 | **Extract `AppBuilder` library** from `qpedia-api/src/main.rs`. Expose `from_env() -> AppBuilder`, `.with_route()`, `.with_state_extension::<T>()`, `.with_auth_provider()`, `.with_event_sink()`, `.serve()`. OSS users see no behavior change. | qpedia | ⚪ |
| 0.2 | Add **`EventSink` trait** + integrate at audit / observability call sites. Default sink = the current tracing log. | qpedia | ⚪ |
| 0.3 | Add **`TenantHook` trait** — fires on tenant create / update / delete. Default impl is a no-op. | qpedia | ⚪ |
